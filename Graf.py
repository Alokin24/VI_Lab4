...
BIdistance = distance(B, I)
BCdistance = distance(B, C)
HIdistance = distance(H, I)
IJdistance = distance(I, J)
FCdistance = distance(F, C)
GCdistance = distance(G, C)
CDdistance = distance(C, D)
FGdistance = distance(F, G)
FJdistance = distance(F, J)
KGdistance = distance(K, G)
LKdistance = distance(L, K)
DEdistance = distance(D, E)
DLdistance = distance(D, L)
LMdistance = distance(L, M)
...
locationsBezStanica1 = copy.deepcopy(graph.dict)
locationsBezStanica2 = copy.deepcopy(graph.dict)


# pat preku Stanica1 ne treba da ja sodrzhi Stanica2
# zatoa od celiot graf ja brisheme Stanica2
del locationsBezStanica1[Stanica2]
# obratnoto vazhi i za Stanica2
del locationsBezStanica2[Stanica1]


graphZaStanica1 = UndirectedGraph(locationsBezStanica2)
graphZaStanica2 = UndirectedGraph(locationsBezStanica1)

odPocetokDoStanica1 = GraphProblem(Pocetok, Stanica1, graphZaStanica1)
odPocetokDoStanica2 = GraphProblem(Pocetok, Stanica2, graphZaStanica2)
odStanica1DoKraj = GraphProblem(Stanica1, Kraj, graphZaStanica1)
odStanica2DoKraj = GraphProblem(Stanica2, Kraj, graphZaStanica2)

# print(greedy_best_first_graph_search(odPocetokDoStanica1).solution())
# print(greedy_best_first_graph_search(odStanica1DoKraj).solution())
# print(greedy_best_first_graph_search(odPocetokDoStanica2).solution())
# print(greedy_best_first_graph_search(odStanica2DoKraj).solution())

ans1 = [Pocetok, ] + uniform_cost_search(odPocetokDoStanica1).solution() + uniform_cost_search(odStanica1DoKraj).solution()
ans2 = [Pocetok, ] + uniform_cost_search(odPocetokDoStanica2).solution() + uniform_cost_search(odStanica2DoKraj).solution()
...
for i in range(0, len(ans1) -1):
    cost1 += graph.get(ans1[i], ans1[i+1])

for i in range(0, len(ans2)-1):
    # try:
        cost2 += graph.get(ans2[i], ans2[i+1])
    # except:
    #     print(ans2[i], ans2[i+1])
